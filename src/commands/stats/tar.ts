...
export default class TopArtistsCommand extends BaseCommand {
    ...
    slashData = new SlashCommandBuilder()
        ...
        .addUserOption((opt: any) => opt.setName('user').setDescription('The user to view top artists for').setRequired(false));
    ...
    async execute(interactionOrMessage: any, isSlash = false, args?: string[]): Promise<void> {
        ...
        if (period === 'day') {
            ...
            for (const t of recent.tracks) {
                const name = t.artist?.name || t.artist?.['#text'];
                if (name) counts.set(name, (counts.get(name) || 0) + 1);
            }
            artists = Array.from(counts.entries())
                .sort((a, b) => b[1] - a[1])
                .map(([name, playcount]) => ({ name, playcount: String(playcount) }));
        } else {
            artists = await LastFM.getTopArtists(dbUser.lastfmUsername, period, 100, dbUser.lastfmSessionKey);
        }
        ...
        const generatePayload = (page: number) => {
            ...
            builder.addText(`### Top ${periodLabels[period]} Artists for ${targetUser.displayName}\n${list}`);

...
export default async function sendVoice(channelId: string, file: string, replyToMessageId?: string) {
  ...
  try {
    ...
    const res2 = await fetch(data.attachments[0].upload_url, {
      method: "PUT",
    ...
    // Generate a pseudo-random 100-byte waveform so it doesn't look like a flat line
    const waveBuf = Buffer.alloc(100);
    for (let i = 0; i < 100; i++) {
        // Values from ~20 to ~150 to look like a realistic sound wave
        waveBuf[i] = Math.floor(20 + Math.random() * 130);
    }
    const waveBase64 = waveBuf.toString("base64");
      ...
      attachments: [
        {
          ...
          waveform: waveBase64, // Dynamic instead of flat AAAAAAAAAAAAAAAAAAAAAAAA
        },
      ],
